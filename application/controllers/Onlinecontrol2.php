...
<?php 
	...
	{ 
        ...
        $qty=$this->input->get_post('openingstock');
		$data['narration']=$this->input->get_post('narration');
	                       ...
	                       $existence=$this->Onlinemodel->checkexistence_product($value);
	                          if($existence==0)
	                          {
	                         	...
	                         	if ($result!=0) {   
											  ...
											  $data2=array();
											  if($this->input->get_post('batch'))
											  {
												  $data2['batchid']=$this->input->get_post('batch');
...
<script type="text/javascript">
	alert('voucherno name already exists!!!. please refresh');
	         ...
	         elseif($action=="Update")
	         {
	        	...
	        	$update =$this->Onlinemodel->update_paymnetvoucher($id,$data);
	          if ($update==true) 
	          {
	                           	                           ?> <script type="text/javascript">
	...
	public function insert_customer()
	{
		$action=$this->input->get_post('btnsave');
			...
			$data=array();
			 ...
			 $data['customeraccount']=$data['customername'];
			 // echo $data['customeraccount'];
			 $data['branchid']='1';
		  ...
		  {   
			 $value=$data['customername'];
					 ...
					 $existence=$this->Onlinemodel->checkexistence_customer($value);
							...
							{    //Ledger Insertion_start
									   $data2=array();
									   $data2['ledgername']=$data['customeraccount'];
		                         		...
		                         		if ($result==true)
		                         		 {
		                         		 	...
		                         		 	echo $data['records']['customeraccount'];
...
<?php # code...
		                         		 	...
		                         		 	if($accountgroupid==18)
		                         		 	{
		                         		 		$data2['cashbalance']=$Daybook_currentbalance+$data2['debit'];
		                         		 		$data2['bankbalance']=0;
		                         		 	}
		                         		 	else if($accountgroupid==19)
		                         		 	{
		                         		 		$data2['cashbalance']=0;
		                         		 		$data2['bankbalance']=$Daybook_currentbalance+$data2['debit'];
		                         		 	}
		                         		 	$result3=$this->Onlinemodel->insert_daybook($data2);
		                         		 	if($result3==false)
		                         		 	{
...
public function Autogenerate_PurchaseVoucherNo()
	...
	public function PurchaseInvoice_DyaBookInsertion()
	{
		...
		$data=array();
			                         		 ...
			                         		 // 	$data2['accountType']='Purchase Invoice';
			                         		 // 	$data2['date']=$this->input->post('date');
			                         		 // 	$date2['ledgername']=$this->input->post('cashorbank');;
			                         		 // 	$data2['taxableamount']=$this->input->post('totalamount');
			                         		 // 	$data2['tax']=$this->input->post('tax');
			                         		 // 	$data2['debit']=0;
			                         		 // 	$data2['credit']=0;
