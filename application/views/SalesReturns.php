    ...
    try
    {
			...
			for (var n = 1; n < table.rows.length; n++)
      {
        ...
        else
        {
          ...
          {
            return this.value == val;}).data('purchaserate');
        ...
        <div class="table table-responsible">
			...
			branch.onchange=function ()
			{
				...
				$.ajax({
		  			type: "POST",
		  			dataType:"json",
		  			url:"<?php echo base_url();?>index.php/Salescontrol/autogenerate_ret",
		  			data: {
			  			'branchid':branchid
					},
					success: function (result) {
...
<script type="text/javascript">
  $('#addrow').on('click', function () {
			...
			<script type="text/javascript">
				$(document).ready(function () {
					...
					$('#btnsave').on('click', function () {
						...
						var returnno = document.getElementById("returnno").value;
						...
						try {
							$.ajax({
								type: "POST",
								// dataType:"json",
								url: "<?php echo base_url() ?>index.php/Salescontrol/insert_salesreturnmaster",
								data: {
									'returnno': returnno,
									'customer': customer,
									'salesdate': salesdate,
								...
								success: function (data) {
									...
									for (var i = 1; i < table.rows.length-1; i++) {
										...
										$.ajax({
											type: "POST",
											url: "<?php echo base_url() ?>index.php/Salescontrol/insert_salesreturndetails",
											data: {
												'salesreturnid':data,
												'returnno': returnno,
												'salesdate': salesdate,
									...
									alert(myJSON);
                                           ...
                                           $.ajax({
												type: "POST",
												url: "<?php echo base_url() ?>index.php/Salescontrol/SalesReturn_DayBookInsertion",
												data: {
													'returnno': returnno,
													'salesdate': salesdate,
													'totalamount': totalamount,
    ...
    $('#dataTable1').append(newTr);
                ...
                //         $.ajax({
								// 				type: "POST",
								// 				url: "<?php echo base_url() ?>index.php/Salescontrol/SalesReturn_AccountInsertion",
								// 				data: {
								// 					'totalamount': totalamount,
								// 					'tax': taxamount,
								// 					'paidcash': paidcash,
