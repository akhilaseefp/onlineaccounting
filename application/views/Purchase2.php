...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$('#dataTable1').on('change', '.productcode2', function () {
			...
			if (a == 1)
			{
				 code = $('#productcode1').val();
				
				// var code =document.getElementById('productcode1').innerHTML();
			}
			else
			{
				code =$('#productcode' + a).val();
				
								// var code =document.getElementById('productcode' + a).innerHTML();
			}
			...
			<div class="box-body">
				<div class="row">
					...
					<div class="col-md-4">
						...
						<input type="text" class="form-control" list="supplier1" required autocomplete="off" id="supplier" name="supplier">
						<datalist id="supplier1">
							<?php foreach($supplier->result() as $gp){
                                      ?>
							<option value="<?php echo $gp->suppliername;?>" data-supplierbalance="<?php echo $gp->currentbalance;?>"
								label=" <?php echo $gp->supplierid;?>"></option>
							<?php  } ?>
				...
				<div class="row">
					...
					<div class="col-md-4">
						...
						<input type="datetime" name="invoicedate" class="form-control" id="invoicedate"
							value="<?php 
							$d=strtotime("now");$e=strtotime("+4 hours 30 minutes",$d); echo date("Y-m-d H:i:s",$e);?>">
					</div>
					<div class="col-md-4">
				...
				<div class="table table-responsible">
					<TABLE id="dataTable1" border="1" class="table table-striped table-hover">
						...
						<tbody id="dataTable" width="100%" border="1" class="table table-hover">
							<tr>
								<td>1</td>
								<td> <input type="text" class="form-control productcode2" required list="name" autocomplete="off"
                   ...
                   ?>
							<option value="<?php echo $gp->pdt_name;?>" label ="<?php echo $gp->pdt_code;?>" data-tax="<?php echo $gp->tax;?>" data-taxlow="<?php echo $gp->taxlow;?>" data-taxhigh="<?php echo $gp->taxhigh;?>" data-slab="<?php echo $gp->slab;?>" data-purchaserate="<?php echo $gp->purchaserate;?>" data-hsncode="<?php echo $gp->hsncode;?>">
								...
								<td>
									<input  type="number" value="1" required class="form-control qty" autocomplete="off" id="qty1"
										name="productcode">
								</td>

								<td> <input type="text" required class="form-control" list="size" autocomplete="off"
										...
										<option value="<?php echo $row->sizevalue; ?>" label="<?php echo $row->sizeid; ?>">
										<?php } ?>
										</datalist>
								...
								<td><input type="text" id="batch" required list="bat" class="form-control" name=""></td>
								<datalist id="bat">
									<?php foreach($batch->result() as $gp){
									?>
...
<script type="text/javascript">
							...
							$.ajax({
									...
									success: function (data) {
										...
										for (var i = 1; i < table.rows.length-1; i++) { 
											...
											no =document.getElementById("dataTable1").rows[i].cells[10].innerHTML;
 ...
 mywindow.document.write('<tr><td>'+n+'</td><td>'+productname+'</td><td>'+qty+'</td><td>'+unitprice+'</td><td>'+taxamount+'</td><td>'+amount+'</td></tr>');
n++;
										}
